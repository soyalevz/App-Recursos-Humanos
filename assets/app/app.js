...
//  Mobile-first | Alertas 3 meses / prorroga / vencimiento
// ================================================================
...
async function dbLoad()         { return await sbFetch('GET'); }
async function dbSave(w)        { return await sbFetch('POST', w); }
async function dbUpdate(id, w)  { return await sbFetch('PATCH', w, id); }
async function dbDelete(id)     { return await sbFetch('DELETE', null, id); }

// ── DATOS BASE (Excel — solo para primera carga) ────────────
...
// ── FECHAS ──────────────────────────────────────────────────
function daysLeft(fin){ if(!fin)return null; return Math.ceil((new Date(fin+'T00:00:00')-new Date())/86400000); }
...
function ageDays(i){ return Math.floor((new Date()-new Date(i+'T00:00:00'))/86400000); }
...
function fmtLiq(s){ return "$"+Number(s).toLocaleString("es-CL"); }

// ── ALERTAS ─────────────────────────────────────────────────
function getAlerts(w){
  const alerts=[], estado=w.estado||null;
  if(w.tipo!=="plazo_fijo"||estado==="convertido_indefinido") return alerts;
  const age=ageDays(w.inicio), threeM=addMonths(w.inicio,3), dTo3=daysBetween(todayStr(),threeM);
  if(estado!=="prorrogado"){
    if(age>=90) alerts.push({tipo:"indefinido_cumplido",level:"purple",msg:"Cumplio 3 meses el "+fmt(threeM)+" — convertir a indefinido o prorrogar 1 mes",showActions:true});
    else if(dTo3<=15&&dTo3>0) alerts.push({tipo:"indefinido_proximo",level:"blue",msg:"Cumple 3 meses el "+fmt(threeM)+" — faltan "+dTo3+" dia"+(dTo3!==1?"s":""),showActions:true});
  }
  if(estado==="prorrogado"&&w.fin){
    const dl=daysLeft(w.fin);
    if(dl<0) alerts.push({tipo:"prorroga_vencida",level:"danger",msg:"Prorroga vencida hace "+Math.abs(dl)+" dias ("+fmt(w.fin)+") — accion urgente",showActions:true});
    else if(dl<=15) alerts.push({tipo:"prorroga_vence",level:"warn",msg:"Prorroga vence en "+dl+" dia"+(dl!==1?"s":"")+" ("+fmt(w.fin)+") — decidir conversion",showActions:true});
  }
  if(estado!=="prorrogado"){
    const fechaRef=w.venc1||w.venc2||w.fin;
    if(fechaRef){
      const dl=daysLeft(fechaRef);
      if(dl<0) alerts.push({tipo:"vencido",level:"danger",msg:"Contrato vencido hace "+Math.abs(dl)+" dias ("+fmt(fechaRef)+")",showActions:false});
      else if(dl<=15) alerts.push({tipo:"vence_pronto",level:"warn",msg:"Contrato vence en "+dl+" dia"+(dl!==1?"s":"")+" — "+fmt(fechaRef),showActions:false});
    }
  }
  return alerts;
}
...
async function marcarEstado(id, estado){
  ...
  if(estado==="prorrogado"){
    ...
    showToast("Prorroga registrada. Nueva fecha: "+fmt(workers[idx].fin));
  }
  if(estado==="convertido_indefinido"){
    workers[idx].tipo="indefinido"; workers[idx].fin=null;
    showToast(workers[idx].nombre+" convertido a indefinido");
  }
  try{ await dbUpdate(id, {estado:workers[idx].estado, tipo:workers[idx].tipo, fin:workers[idx].fin||null}); }
  catch(e){ console.error(e); showToast("Error al guardar en la nube","error"); }
  closeDetail(); render();
}
...
function populateCCFilter(){
  const sel=document.getElementById("filtro-cc"); if(!sel)return;
  const cur=sel.value, ccs=[...new Set(workers.map(w=>w.cc))].sort();
  sel.innerHTML="<option value="">Todas las obras</option>"+ccs.map(c=>"<option value=""+c+"">"+c+"</option>").join("");
...
// ── RENDER ──────────────────────────────────────────────────
function render(){
  populateCCFilter(); updateNavBadge();
...
function updateNavBadge(){
  const n=totalAlerts(), b=document.getElementById("nav-badge");
...
function renderTrabajadores(filtered){
  ...
  if(!ccs.length){mc.innerHTML="<div class="empty"><i class="ti ti-users"></i>Sin trabajadores para mostrar</div>";return;}
  let html="";
  ccs.forEach(cc=>{
    const group=byCC[cc], alerts=group.filter(hasAlert).length, cfg=ccCfg(cc);
    html+="<div class="group-label"><span><span class="group-icon" style="background:"+cfg.bg+";color:"+cfg.tx+""><i class="ti "+cfg.icon+""></i></span>"+cc+" <span style="font-weight:400;color:var(--txt2)">("+group.length+")</span></span>"+(alerts?"<span class="badge b-danger"><i class="ti ti-alert-triangle" style="font-size:10px"></i> "+alerts+"</span>":"")+"</div>";
    group.forEach(w=>{
      const als=getAlerts(w), top=als[0]||null;
      let accion="",accionCls="";
      if(top){
        if(top.tipo==="indefinido_cumplido"){accion="Convertir/Prorrogar";accionCls="purple";}
        else if(top.tipo==="indefinido_proximo"){accion="3M en "+daysBetween(todayStr(),addMonths(w.inicio,3))+"d";accionCls="blue";}
        else if(top.tipo==="prorroga_vence"){accion="Prorroga vence "+daysLeft(w.fin)+"d";accionCls="warn";}
        else if(top.tipo==="prorroga_vencida"){accion="Prorroga vencida";accionCls="danger";}
        else if(top.tipo==="vence_pronto"){accion="Vence en "+daysLeft(w.venc1||w.venc2||w.fin)+"d";accionCls="warn";}
        else if(top.tipo==="vencido"){accion="Vencido";accionCls="danger";}
      }
      html+="<div class="worker-card" onclick="openDetail("+w.id+")">"
        +"<div class="wc-row1"><div><div class="wc-name">"+w.nombre+"</div><div class="wc-rut">"+w.rut+"</div><div class="wc-cargo">"+(w.cargo||"")+"</div></div>"
        +"<div class="wc-right"><div class="wc-liquido">"+fmtLiq(w.liquido)+"</div>"+estadoBadge(w)+"</div></div>"
        +"<div class="wc-row2"><div class="wc-info"><span class="badge" style="background:var(--bg3);color:var(--txt2)">"+(w.horario==="art22"?"Art.22":"Jornada ord.")+"</span>"+(w.inicio?"<span style="font-size:10px;color:var(--txt2)">Ingreso: "+fmt(w.inicio)+"</span>":"")+"</div>"+(accion?"<span class="wc-accion "+accionCls+"">"+accion+"</span>":"")+"</div></div>";
    });
  });
  html+="<div style="text-align:center;padding:16px 0 8px"><button class="add-btn" onclick="openForm()"><i class="ti ti-plus"></i> Agregar trabajador</button></div>";
  mc.innerHTML=html;
}
...
function renderAlertas(){
  ...
  if(!all.length){mc.innerHTML="<div class="sec-title"><i class="ti ti-bell" style="color:var(--danger)"></i> Alertas activas</div><div class="empty"><i class="ti ti-circle-check" style="color:var(--ok)"></i>Sin alertas — todo en orden</div>";return;}
  const grupos=[
    {key:"indefinido_cumplido",label:"Conversion a indefinido requerida",color:"var(--purple)"},
    {key:"indefinido_proximo",label:"Proximos a cumplir 3 meses",color:"var(--teal)"},
    {key:"prorroga_vencida",label:"Prorroga vencida — urgente",color:"var(--danger)"},
    {key:"prorroga_vence",label:"Prorroga vence pronto",color:"var(--warn)"},
    {key:"vencido",label:"Contratos vencidos",color:"var(--danger)"},
    {key:"vence_pronto",label:"Contratos vencen en 15 dias",color:"var(--warn)"},
  ];
  ...
  grupos.forEach(g=>{
    const items=all.filter(x=>x.a.tipo===g.key); if(!items.length)return;
    ...
    items.forEach(({w,a})=>{
      ...
      if(["indefinido_cumplido","indefinido_proximo"].includes(a.tipo)){
        btns="<button class="action-btn purple" onclick="marcarEstado("+w.id+",'prorrogado')"><i class="ti ti-clock-plus" style="font-size:12px"></i> Prorrogar 1 mes</button><button class="action-btn ok" onclick="marcarEstado("+w.id+",'convertido_indefinido')"><i class="ti ti-infinity" style="font-size:12px"></i> Convertir a indefinido</button>";
      }
      if(["prorroga_vence","prorroga_vencida"].includes(a.tipo)){
        btns="<button class="action-btn ok" onclick="marcarEstado("+w.id+",'convertido_indefinido')"><i class="ti ti-infinity" style="font-size:12px"></i> Convertir a indefinido</button>";
      }
      html+="<div class="alert-box "+a.level+""><div class="alert-icon"><i class="ti ti-"+(iconMap[a.level]||"alert-triangle")+"" ></i></div><div style="flex:1"><div class="alert-name">"+w.nombre+" <span style="font-weight:400;font-size:11px">"+w.rut+"</span></div><div class="alert-det">"+w.cc+" · "+(w.cargo||"")+" · "+fmtLiq(w.liquido)+"</div><div class="alert-det">Ingreso: "+fmt(w.inicio)+"</div><div class="alert-msg">"+a.msg+"</div>"+(btns?"<div class="alert-actions">"+btns+"</div>":"")+"</div></div>";
    });
  });
  mc.innerHTML=html;
}
...
function renderResumen(){
  const mc=document.getElementById("main-content");
  const alertas=totalAlerts(), prorrog=workers.filter(w=>w.estado==="prorrogado").length, convert=workers.filter(w=>w.estado==="convertido_indefinido").length;
  const byCC={};
  workers.forEach(w=>{byCC[w.cc]=(byCC[w.cc]||0)+1;});
  mc.innerHTML="<div class="sec-title"><i class="ti ti-chart-bar" style="color:var(--blue)"></i> Resumen general</div>"
    +"<div class="stat-grid"><div class="stat-card"><div class="stat-num">"+workers.length+"</div><div class="stat-label">Total trabajadores</div></div><div class="stat-card"><div class="stat-num" style="color:var(--warn)">"+alertas+"</div><div class="stat-label">Con alertas</div></div><div class="stat-card"><div class="stat-num" style="color:var(--teal)">"+prorrog+"</div><div class="stat-label">Prorrogados</div></div><div class="stat-card"><div class="stat-num" style="color:var(--ok)">"+convert+"</div><div class="stat-label">Convertidos</div></div></div>"
    +"<div class="sec-title" style="margin-bottom:8px"><i class="ti ti-building"></i> Por obra</div>"
    +Object.keys(byCC).sort().map(cc=>{
      const fa=workers.filter(w=>w.cc===cc&&hasAlert(w)).length, cfg=ccCfg(cc);
      return "<div class="cc-row"><div class="cc-ico" style="background:"+cfg.bg+";color:"+cfg.tx+""><i class="ti "+cfg.icon+""></i></div><span class="cc-name">"+cc+"</span><span class="cc-cnt">"+byCC[cc]+" pers.</span>"+(fa?"<span class="cc-alert">"+fa+" alerta"+(fa!==1?"s":"")+"</span>":"")+"</div>";
    }).join("");
}
...
function openDetail(id){
  ...
  let alertHtml=als.map(a=>"<div class="alert-box "+a.level+"" style="margin:8px 16px 0"><div class="alert-icon"><i class="ti ti-alert-triangle"></i></div><div><div class="alert-msg">"+a.msg+"</div></div></div>").join("");
  const rows=[["Cargo",w.cargo||"—"],["Obra / CC",w.cc],["Tipo",w.tipo==="indefinido"?"Indefinido":w.tipo==="obra_faena"?"Obra/Faena":"Plazo fijo"],["Horario",w.horario==="art22"?"Art. 22":"Jornada ordinaria"],["Ingreso",fmt(w.inicio)],["Liquido",fmtLiq(w.liquido)],["Vencimiento 1",fmt(w.venc1)||"—"],["Vencimiento 2",fmt(w.venc2)||"—"],["Correo",w.correo||"—"],["Telefono",w.telefono||"—"],["Notas",w.notas||"—"]];
  document.getElementById("detail-content").innerHTML="<div class="detail-hero"><div class="detail-avatar">"+initials(w.nombre)+"</div><div class="detail-hero-info"><div class="name">"+w.nombre+"</div><div class="sub">"+w.rut+"</div><div style="margin-top:4px">"+estadoBadge(w)+"</div></div></div>"+alertHtml+rows.map(([l,v])=>"<div class="detail-row"><span class="detail-label">"+l+"</span><span class="detail-val">"+v+"</span></div>").join("");
  document.getElementById("detail-actions").innerHTML="<button class="btn" onclick="closeDetail()">Cerrar</button><button class="btn primary" onclick="closeDetail();openForm("+w.id+")" ><i class="ti ti-edit"></i> Editar</button><button class="btn danger-btn" onclick="closeDetail();deleteWorker("+w.id+")"><i class="ti ti-trash"></i></button>";
...
function openForm(id){
  ...
  if(f&&!KNOWN_CCS.includes(f.cc)){ccSel.value="otro";document.getElementById("grp-cc-manual").style.display="";document.getElementById("f-cc-manual").value=f.cc;}
  else{ccSel.value=f?f.cc:"TALLER";document.getElementById("grp-cc-manual").style.display="none";}
  toggleFechaFin();
  document.getElementById("modal-overlay").classList.add("open");
}
function closeForm(){ document.getElementById("modal-overlay").classList.remove("open"); }
function toggleFechaFin(){ const t=document.getElementById("f-tipo").value; document.getElementById("grp-fin").style.display=(t==="indefinido"||t==="obra_faena")?"none":""; }
...
async function saveWorker(){
  ...
  try{
    if(editingId){
      ...
      const newId=nextId++;
      const newWorker={id:newId,...data,estado:null};
      workers.push(newWorker);
      await dbSave(newWorker);
      showToast("Trabajador agregado");
    }
    closeForm(); render();
  } catch(e){
    console.error(e);
    showToast("Error al guardar. Revisa tu conexion.","error");
  } finally{
...
async function deleteWorker(id){
  ...
  if(!w||!confirm("Eliminar a "+w.nombre+"? Esta accion no se puede deshacer."))return;
  workers=workers.filter(x=>x.id!==id);
  render();
  try{ await dbDelete(id); showToast("Trabajador eliminado"); }
...
async function init(){
  ...
  try {
    ...
    if(!data||data.length===0){
      showToast("Primera carga: subiendo datos a la nube...");
      for(const w of BASE_WORKERS){
        await dbSave(w);
      }
      data=BASE_WORKERS;
      showToast("87 trabajadores cargados correctamente");
    }
    workers=data;
    nextId=Math.max(...workers.map(w=>Number(w.id)),100)+1;
  } catch(e){
    console.error("Error Supabase:",e);
    showToast("Sin conexion — mostrando datos locales","error");
    const local=localStorage.getItem("salfate_backup");
    workers=local?JSON.parse(local):BASE_WORKERS.map(w=>({...w}));
  }
  localStorage.setItem("salfate_backup",JSON.stringify(workers));
